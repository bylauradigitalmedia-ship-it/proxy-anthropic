...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET") return send(res, 200, "Proxy OK");

  const url = req.url.split("?")[0];
  const body = await readBody(req);

  if (url === "/generate") {
    if (!ANTHROPIC_KEY) return send(res, 500, { error: "Cle API manquante" });
    const { compte, sujet, format, objectif, ton, details } = body;
    if (!sujet || !compte) return send(res, 400, { error: "Parametres manquants" });
    ...
    try {
      ...
      if (d.type === "error") return send(res, 500, { error: d.error.message });
      const raw = (d.content || []).filter(b => b.type === "text").map(b => b.text).join("");
      const s = raw.indexOf("{"), e = raw.lastIndexOf("}");
      if (s === -1) return send(res, 500, { error: "JSON introuvable" });
      return send(res, 200, JSON.parse(raw.slice(s, e + 1)));
    } catch (err) {
  ...
  if (url === "/notion-add") {
    if (!NOTION_KEY) return send(res, 500, { error: "Token Notion manquant" });
    const { compte_key, sujet, format, date, heure, hook, legende, hashtags, premier_commentaire, story_relance, conseil_visuel } = body;
    const dbId = DBS[compte_key];
    if (!dbId) return send(res, 400, { error: "Client inconnu: " + compte_key });
    ...
    try {
      ...
      if (schema.object === "error") return send(res, 400, { error: schema.message });
      ...
      if (created.object === "error") return send(res, 400, { error: created.message });
      return send(res, 200, { success: true, notion_url: created.url });
    } catch (err) {
      return send(res, 500, { error: err.message });
    }
  }
...
server.listen(PORT, () => console.log("Serveur OK sur port " + PORT));
